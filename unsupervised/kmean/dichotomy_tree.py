...
我讲不下去了，看代码吧
最后的一个赋值竟然出问题了，逻辑上没问题啊。数据也没问题，我从网上拷贝了一份也是错的
'''
from numpy import *
...
def dichmeans(dataset, k, distmeans=distance):
    ...
    while len(centlist) < k:
        ...
        # print(bestclusass[nonzero(bestclusass[:, 0].A == 1)[0], 0] )
        # print(bestclusass[nonzero(bestclusass[:, 0].A == 0)[0], 0])
        centlist[bestcenttosplit] = bestnewcentes[0, :]
        centlist.append(bestnewcentes[1, :])
        print('***',clusterassment[nonzero(clusterassment[:, 0].A == bestcenttosplit)[0], :],'@@')
        print('&&&',bestclusass,'####')
